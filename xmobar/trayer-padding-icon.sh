...
# Calculate number of spaces needed based on the approximation that each space is ~13 pixels wide
num_spaces=$((width / 13 + 1))

# Generate the spaces
spaces=$(printf '%*s' "$num_spaces")

# Output the spaces directly
echo "$spaces"
# Output the spaces wrapped in a text tag for xmobar
echo "<fc=transparent>$spaces</fc>"
